...
# Locks the parallel-refresh contract: every configured source is refreshed
# concurrently through the bounded worker pool, each verified independently via
# the failover policy. A broken source never blocks the others, mirror failover
# still applies, and selection (priority/version) is deterministic regardless of
# completion order.

# A small HTTP server that sleeps before every response, to make the parallel
# speedup measurable without depending on network conditions.
...
# --- shared good / bad HTTP repos (same helpers as section 34) ---
makePrioRepo fo-good "fogood" "fo-good-fixture" "hello" "1.0.0"
echo "hello 1.0.0" > fo-good/packages.index
rm -rf fo-bad
cp -r fo-good fo-bad
...
# Case 1: two repos with a slow first mirror each; parallel must beat serial.
# Both sources point only at the slow server (no fast fallback), so serial would
# be ~2 * DELAY and parallel ~1 * DELAY.
rm -rf fo-slowroot
...
# Case 2: one broken repo does not block the others.
FO_BAD_PORT=$(python3 -c 'import socket;s=socket.socket();s.bind(("",0));print(s.getsockname()[1]);s.close()')
"$(cd "$(dirname "$0")" && pwd)/../build/meow-server" serve ./fo-bad --port "$FO_BAD_PORT" >/tmp/fo-bad.log 2>&1 &
FO_BAD_PID=$!
...
# Case 3: mirror-group failover still applies under parallel refresh.
...
# Case 4: different repository_id never share a cache directory.
...
# Case 5: deterministic selection across repeated parallel refreshes.
...
echo ""
echo "=== package groups ==="
# Locks the package-group contract: groups are expansion aliases (local
# policy in meow.toml), install atomically via the shared staging path, and
# never create a synthetic database entity.
...
# Case: parse group config + list groups.
...
# Case: install group -> members installed (atomic single transaction).
...
# Case: group dependency closure -- installing "app" pulls hello + libfoo.
...
# Case: group install is atomic -- one unresolvable member means nothing lands.
...
# Case: duplicate group name rejected at config load.
...
# Extra: reserved-name group rejected at config load.
